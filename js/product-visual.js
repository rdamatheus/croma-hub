...
export function renderProductVisual(item) {
  ...
  var configuredSrc = visual.produto || item.imagem || '';
  var productSrc = isAssetPath(configuredSrc) ? configuredSrc : '';
  ...
  var placeholder = '<div class="media-brand-placeholder" aria-hidden="true"><span class="media-brand-mark">' + escapeHtml(initials(item.nome)) + '</span><small>Croma</small></div>';
  var productLayer = productSrc
    ? '<img class="media-product ' + (mode === "recorte" ? "media-cutout" : "media-photo") + '" src="' + escapeHtml(productSrc) + '" alt="' + title + '" loading="lazy" decoding="async" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><div class="media-brand-placeholder" hidden aria-hidden="true"><span class="media-brand-mark">' + escapeHtml(initials(item.nome)) + '</span><small>Croma</small></div>'
    : placeholder;
  var backgroundLayer = backgroundSrc
    ? '<img class="media-background-image" src="' + escapeHtml(backgroundSrc) + '" alt="" loading="lazy" decoding="async" onerror="this.remove()">'
    : '';
  var badgeLayer = badges.length
    ...
    : '';
  var shadowLayer = mode === "recorte" && productSrc && visual.sombra !== false ? '<span class="media-shadow" aria-hidden="true"></span>' : '';
