import { getStaffSession } from './interno-auth.js';
...
function setAccessState(title,message,{retry=false}={}){
  const titleEl=el('supplierAccessTitle');
  const messageEl=el('supplierAccessMessage');
  const actions=el('supplierAccessActions');
  if(titleEl)titleEl.textContent=title;
  if(messageEl)messageEl.textContent=message;
  if(actions)actions.hidden=!retry;
}

function authTimeout(){
  return new Promise((_,reject)=>setTimeout(()=>reject(new Error('Tempo limite ao validar a sessão.')),AUTH_TIMEOUT_MS));
}

async function authorizePage(){
  setAccessState('Validando acesso…','Aguarde enquanto confirmamos sua sessão e carregamos a área de fornecedores.');
  try{
    const session=await Promise.race([getStaffSession(),authTimeout()]);
    if(!session?.user){
      const next=location.pathname+location.search+location.hash;
      location.replace(`/interno/?next=${encodeURIComponent(next)}`);
      return null;
    }
    if(!session.profile){
      setAccessState('Acesso indisponível','Sua conta não está habilitada para a área interna da Croma.',{retry:true});
      return null;
    }
    if(!ALLOWED_ROLES.has(session.profile.role)){
    ...
    console.error('Falha ao validar acesso aos fornecedores:',error);
    setAccessState('Não foi possível carregar a área de fornecedores','A validação da sessão demorou demais ou encontrou um erro. Tente novamente.',{retry:true});
    return null;
  }
}

el('supplierRetryAccess')?.addEventListener('click',()=>location.reload());
const session=await authorizePage();
if(!session){
  // Mantém o estado de erro/redirect visível, sem deixar a página em branco.
} else {
  ...
  async function load(){
    ...
    el('pageStatus').className='status';
    try{
      ...
      console.error(error);
      el('pageStatus').textContent=error.message||'Falha ao carregar fornecedores.';
      el('pageStatus').className='status bad';
      el('supplierRows').innerHTML='<tr><td colspan="8" class="loading-skeleton">Não foi possível carregar os fornecedores. Atualize a página ou tente novamente em instantes.</td></tr>';
    }
  }
