...
try{
  ...
  if(s.error||l.error)throw s.error||l.error;
  const segs=s.data||[],links=l.data||[],productIds=[...new Set(links.map(x=>x.product_id).filter(Boolean))],products=[];
  for(let i=0;i<productIds.length;i+=180){
    const {data,error}=await supabase.from('products')
      .select('id,nome,slug,metadata,product_type,ativo')
      .in('id',productIds.slice(i,i+180))
      .eq('ativo',true);
    if(error)throw error;
    products.push(...(data||[]));
  }
  const roots=segs.filter(x=>!x.parent_id);
