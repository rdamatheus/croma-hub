...
let families=[],categories=[],categoryCounts=new Map(),page=1,selectedCategory='',searchTerm=(params.get('q')||'').trim().toLowerCase(),renderSeq=0,searchTimer=null;
...
function categoryCount(id){return Number(categoryCounts.get(id)?.subtree||0)}
function categoryHasItems(id){return categoryCount(id)>0}
function familyCount(id){return categories.filter(c=>c.family_id===id).reduce((sum,c)=>sum+Number(categoryCounts.get(c.id)?.direct||0),0)}
...
function renderShell(){
  ...
  const search=root.querySelector('#productSearch');search.value=searchTerm;search.addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(()=>{searchTerm=search.value.trim().toLowerCase();page=1;renderProducts()},250)});
...
async function renderProducts(){
  ...
  const seq=++renderSeq;
  const categoryIds=selectedCategory?[...descendantIds(categories,selectedCategory)]:[];
  box.innerHTML='<div class="public-empty">Carregando produtos...</div>';
  try{
    const result=await loadPublicCatalogPage('produto',{requirePublished:false,categoryIds,search:searchTerm,page,pageSize:PAGE_SIZE});
    if(seq!==renderSeq)return;
    const total=result.total,pages=Math.max(1,Math.ceil(total/PAGE_SIZE));
    if(page>pages){page=pages;return renderProducts()}
  ...
  }catch(e){console.error(e);if(seq===renderSeq)box.innerHTML='<div class="public-empty">Não foi possível carregar os produtos agora.</div>'}
}

try{
  const data=await loadPublicCatalogMeta('produto',{requirePublished:false});families=data.families;categories=data.categories;categoryCounts=data.counts;
  if(selectedProductRef){
    const product=await loadPublicCatalogItem('produto',selectedProductRef,{requirePublished:false});
