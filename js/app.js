...
function setupShowcaseControls(){
  ...
  wrap.innerHTML=`<div class="showcase-copy"><p>Uma seleção ligada aos cadastros reais da Croma. Alterações de dados passam a refletir na vitrine sem cards avulsos.</p></div><div class="showcase-switch" data-showcase-switch role="tablist" aria-label="Alternar vitrine"><span class="showcase-switch-thumb" aria-hidden="true"></span><button type="button" class="active" data-showcase-type="produtos" role="tab" aria-selected="true">Produtos</button><button type="button" data-showcase-type="servicos" role="tab" aria-selected="false">Serviços</button></div>`;
  heading?.after(wrap);
...
function renderShowcase(){
  ...
  if(!itens.length){
    ...
    grid.innerHTML=`<div class="empty-state showcase-empty">Ainda não há ${label} disponíveis para esta vitrine.</div>`;
    return;
  }
  grid.innerHTML=itens.map(item=>{
    const action=item.tipo==='servico'?'Ver serviço':'Ver produtos';
    const description=item.descricao?`<p>${esc(item.descricao)}</p>`:'<p class="showcase-muted">Item vinculado ao cadastro oficial da Croma.</p>';
