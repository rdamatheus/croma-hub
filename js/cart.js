(() => {
  ...
  const saveCart = next => { items = next; localStorage.setItem(CART_KEY, JSON.stringify(items)); render(); window.dispatchEvent(new CustomEvent('croma:cartchange',{detail:[...items]})); };
  let items = loadCart();
  ...
  function draftRef(){
    ...
    const rand = (crypto.randomUUID ? crypto.randomUUID().replaceAll('-','') : Math.random().toString(36).slice(2)).slice(0,8).toUpperCase();
    ref = `RASC-${date}-${rand}`;
  ...
  const shell=document.createElement('div'); shell.className='croma-cart-shell'; shell.hidden=true;
  ...
  function optionsText(opts){return Object.entries(opts||{}).filter(([,v])=>v!==''&&v!=null).map(([k,v])=>`${k}: ${v}`).join(' · ')}
  function render(){
    const count=items.reduce((a,x)=>a+Number(x.qty||1),0), total=items.reduce((a,x)=>a+Number(x.total||0),0);
    ...
    shell.querySelector('.croma-cart-list').innerHTML=items.map(x=>`<div class="croma-cart-item"><div><strong>${esc(x.name)}</strong><small>${esc(optionsText(x.options))}</small>${x.fileNames?.length?`<small>Arquivo: ${esc(x.fileNames.join(', '))}</small>`:''}<button class="croma-cart-remove" type="button" data-remove="${esc(x.cartId)}">Remover</button></div><div class="croma-cart-item-price">${x.qty||1} × ${brl(x.unitPrice)}<br>${brl(x.total)}</div></div>`).join('');
  }
  shell.querySelector('.croma-cart-btn').addEventListener('click',()=>shell.classList.toggle('open'));
  shell.querySelector('.croma-cart-clear').addEventListener('click',()=>{if(confirm('Esvaziar o carrinho?'))saveCart([])});
  shell.addEventListener('click',e=>{const b=e.target.closest('[data-remove]');if(b)saveCart(items.filter(x=>x.cartId!==b.dataset.remove))});
  document.addEventListener('click',e=>{if(!shell.contains(e.target))shell.classList.remove('open')});

  window.CromaCart={
    add(item){const qty=Math.max(1,Number(item.qty||1)),unitPrice=Number(item.unitPrice||0);const entry={...item,cartId:uid(),qty,unitPrice,total:item.total!=null?Number(item.total):qty*unitPrice,addedAt:new Date().toISOString()};saveCart([...items,entry]);shell.classList.add('open');window.dispatchEvent(new CustomEvent('croma:itemadded',{detail:entry}));return entry},
    remove(cartId){saveCart(items.filter(x=>x.cartId!==cartId))},
    clear(){saveCart([])},
  ...
  let uploadListEl=null,uploadStatusEl=null,uploadLabelEl=null,authActionsEl=null;

  async function uploadFile(file){
    if(!(file instanceof File)||file.size<=0)throw new Error('Selecione um arquivo válido.');
    if(file.size>MAX_FILE_SIZE)throw new Error(`O arquivo ${file.name} ultrapassa 50 MB.`);
    const {supabase,STORAGE_BUCKET,user}=await authContext();
    if(!user){const err=new Error('LOGIN_REQUIRED');err.code='LOGIN_REQUIRED';throw err}
    const random=(crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random()}`).replaceAll('-','');
    const path=`clientes/${user.id}/rascunhos/${draftRef()}/${Date.now()}-${random.slice(0,8)}-${safeName(file.name)}`;
    const {data,error}=await supabase.storage.from(STORAGE_BUCKET).upload(path,file,{upsert:false,cacheControl:'3600',contentType:file.type||'application/octet-stream'});
    if(error)throw error;
    return{id:`up_${random.slice(0,12)}`,name:file.name,type:file.type||'application/octet-stream',size:file.size,path:data.path,bucket:STORAGE_BUCKET,uploadedAt:new Date().toISOString(),customerId:user.id};
  }
  ...
  function renderUploadList(){
    ...
    if(uploadStatusEl&&list.length){uploadStatusEl.className='croma-upload-status croma-upload-ok';uploadStatusEl.textContent=`${list.length} arquivo(s) pronto(s) para este item.`}
  }

  window.CromaUpload={
    async stage(files){const metas=[];for(const f of files)metas.push(await uploadFile(f));const map=loadMap(),key=pageKey();map[key]=[...(map[key]||[]),...metas];saveMap(map);renderUploadList();return metas},
    ...
    clearCurrent(){const map=loadMap();delete map[pageKey()];saveMap(map);renderUploadList();if(uploadStatusEl){uploadStatusEl.className='croma-upload-status';uploadStatusEl.textContent='Nenhum arquivo anexado para o próximo item.'}},
    consumeCurrent(){const list=this.getForCurrentPage();this.clearCurrent();return list},
    getDraftRef:draftRef
  };

  function shouldInjectUploader(){const p=location.pathname.replace(/\/+$/,'/');return p.startsWith('/servicos/')&&p!=='/servicos/'}
  if(shouldInjectUploader()){
    const main=document.querySelector('main');
    if(main){
      const card=document.createElement('section');card.className='croma-upload-card';
      card.innerHTML=`<h2>Envie seu arquivo</h2><p>Anexe a arte, PDF, imagem ou documento necessário para produzir este serviço.</p><div class="croma-upload-row"><label class="croma-upload-label">📎 Selecionar arquivo(s)<input type="file" multiple></label><span class="croma-upload-status">Verificando sua conta...</span></div><div class="croma-auth-actions" hidden><a class="croma-login" href="/conta/?next=${encodeURIComponent(location.href)}">Entrar</a><a class="croma-signup" href="/cadastro/">Criar conta</a></div><div class="croma-upload-files"></div>`;
      main.appendChild(card);uploadListEl=card.querySelector('.croma-upload-files');uploadStatusEl=card.querySelector('.croma-upload-status');uploadLabelEl=card.querySelector('.croma-upload-label');authActionsEl=card.querySelector('.croma-auth-actions');const input=card.querySelector('input');
      authContext().then(({user})=>{if(user){uploadStatusEl.textContent='Nenhum arquivo anexado para o próximo item.';renderUploadList()}else{uploadStatusEl.className='croma-upload-status croma-upload-error';uploadStatusEl.textContent='Faça login ou crie sua conta para anexar arquivos.';authActionsEl.hidden=false}}).catch(()=>{uploadStatusEl.textContent='Não foi possível verificar sua conta. Atualize a página.'});
      uploadLabelEl.addEventListener('click',async e=>{const {user}=await authContext();if(!user){e.preventDefault();uploadStatusEl.className='croma-upload-status croma-upload-error';uploadStatusEl.textContent='Faça login ou crie sua conta para anexar arquivos.';authActionsEl.hidden=false}});
      input.addEventListener('change',async e=>{const files=[...e.target.files];if(!files.length)return;uploadLabelEl.classList.add('busy');uploadStatusEl.className='croma-upload-status';uploadStatusEl.textContent='Enviando arquivo...';try{await window.CromaUpload.stage(files);authActionsEl.hidden=true}catch(err){uploadStatusEl.className='croma-upload-status croma-upload-error';uploadStatusEl.textContent=err.code==='LOGIN_REQUIRED'?'Faça login ou crie sua conta para anexar arquivos.':'Não foi possível enviar o arquivo. Tente novamente.';if(err.code==='LOGIN_REQUIRED')authActionsEl.hidden=false}finally{uploadLabelEl.classList.remove('busy');e.target.value=''}});
