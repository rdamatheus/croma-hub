  ...
  const enhanceProducts=()=>{
    ...
    const wait=()=>{
      ...
      if(head&&!head.querySelector('.croma-product-tools')){
        ...
        tools.querySelector('#cromaNewProduct').onclick=()=>{const proper=document.querySelector('#newLocalProduct');if(proper){proper.click();return}document.querySelector('#editor')?.scrollIntoView({behavior:'smooth'})};
        tools.querySelector('#cromaProductStatus').onchange=e=>{document.querySelectorAll('.product-row').forEach(r=>{const inactive=r.textContent.toLowerCase().includes('inativo'),v=e.target.value;r.style.display=v==='all'||(v==='inactive'&&inactive)||(v==='active'&&!inactive)?'':'none'})};
      }
      const decorate=()=>{document.querySelectorAll('.product-row').forEach(r=>{if(!r.querySelector('.croma-edit-chip')){const x=document.createElement('span');x.className='croma-edit-chip';x.textContent='Editar';r.appendChild(x)}});stripProductDelete()};decorate();new MutationObserver(decorate).observe(document.body,{childList:true,subtree:true});
      const editor=document.querySelector('#editor');if(editor&&!editor.querySelector('#cromaToggleProduct')){const actions=editor.querySelector('.editor-actions');if(actions){const b=document.createElement('button');b.type='button';b.id='cromaToggleProduct';b.className='btn croma-deactivate';b.textContent='Ativar / Desativar produto';b.onclick=()=>{const ativo=document.querySelector('#ativo');if(!ativo)return;ativo.value=ativo.value==='false'?'true':'false';ativo.dispatchEvent(new Event('change',{bubbles:true}));const status=document.querySelector('#status');if(status)status.textContent=ativo.value==='false'?'Produto marcado como inativo. Clique em Salvar para confirmar.':'Produto reativado. Clique em Salvar para confirmar.'};actions.appendChild(b)}}stripProductDelete();
    };wait();
  };
  const add=()=>{
    const normalized=location.pathname.replace(/\/+$/,'/')||'/';if(normalized==='/interno/')return;
    document.body.classList.add('croma-modern');
    if(!document.querySelector('link[data-croma-modern]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/css/interno-modern.css?v=20260831-5';l.dataset.cromaModern='1';document.head.appendChild(l)}
    if(!document.querySelector('.croma-sidebar')){
      ...
      const header=document.querySelector('.internal-header');if(header&&!header.querySelector('.croma-mobile-menu')){const b=document.createElement('button');b.type='button';b.className='croma-mobile-menu';b.textContent='☰';b.onclick=()=>document.body.classList.toggle('nav-open');header.prepend(b)}
    }
    enhanceProducts();
  };
