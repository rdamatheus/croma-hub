...
function setUrl(family='',category=''){
  ...
  history.replaceState({},'',u.pathname+(u.search?u.search:''));familySlug=family;categorySlug=category;
}

function renderFamilyCard(f){
  const media=familyMedia(f),roots=rootsForFamily(f.id);
  return `<article class="sc-family-card" data-family-card="${esc(f.slug)}"><button type="button" class="sc-family-main" data-family-trigger="${esc(f.slug)}" aria-expanded="false" style="width:100%;padding:0;border:0;background:transparent;text-align:left;cursor:pointer"><div class="sc-family-media">${media?`<img src="${esc(media)}" alt="${esc(f.image_alt||f.nome)}" loading="lazy">`:''}</div><div class="sc-family-body"><h2>${esc(f.nome)}</h2><p>${esc(f.descricao||'Conheça as soluções desta família.')}</p><span class="sc-family-action">${roots.length} categoria(s) ↓</span></div></button><div class="sc-family-detail" data-family-detail></div></article>`;
}

function categoryCard(f,c){
  const media=categoryMedia(c,f),children=childrenFor(c.id),count=categoryServices(c.id).length;
  return `<article class="sc-category-mini"><a href="/servicos/?familia=${encodeURIComponent(f.slug)}&categoria=${encodeURIComponent(c.slug)}" style="text-decoration:none;color:inherit"><div class="sc-category-mini-media">${media?`<img src="${esc(media)}" alt="${esc(c.image_alt||c.nome)}" loading="lazy">`:''}</div><div class="sc-category-mini-copy"><strong>${esc(c.nome)}</strong><span>${esc(c.descricao||'Ver esta categoria.')}</span><small>${children.length} subcategoria(s) · ${count} serviço(s) publicado(s)</small></div></a>${children.length?`<div style="padding:10px 14px 14px;display:flex;flex-wrap:wrap;gap:8px">${children.map(ch=>`<a class="sc-family-close" href="/servicos/?familia=${encodeURIComponent(f.slug)}&categoria=${encodeURIComponent(ch.slug)}" style="text-decoration:none">${esc(ch.nome)}</a>`).join('')}</div>`:''}</article>`;
}

function renderHome(){
  root.innerHTML=`<div class="service-catalog"><div class="sc-intro"><p class="sc-eyebrow">Croma Gráfica</p><h1>Serviços organizados por solução.</h1><p>Navegue por família, categoria e subcategoria. Itens internos, insumos e serviços ainda não publicados não aparecem nesta vitrine.</p></div><div class="sc-family-grid">${families.map(renderFamilyCard).join('')}</div></div>`;
...
async function renderCategory(){
  ...
  const path=categoryPath(categories,families,cat.id);
  const children=childrenFor(cat.id);
  root.innerHTML=`<div class="service-catalog"><div class="sc-breadcrumb"><a href="/">Início</a><span>›</span><a href="/servicos/">Serviços</a>${path.map((x,i)=>i===path.length-1?`<span>›</span><strong>${esc(x.nome)}</strong>`:`<span>›</span><a href="${i===0?`/servicos/?familia=${encodeURIComponent(fam.slug)}`:`/servicos/?familia=${encodeURIComponent(fam.slug)}&categoria=${encodeURIComponent(x.slug||'')}`}">${esc(x.nome)}</a>`).join('')}</div><section class="sc-family-hero">${familyMedia(fam)?`<img src="${esc(familyMedia(fam))}" alt="${esc(fam.image_alt||fam.nome)}">`:''}<div class="sc-family-hero-copy"><p class="sc-eyebrow">${esc(fam.nome)}</p><h1>${esc(cat.nome)}</h1><p>${esc(cat.descricao||'Soluções disponíveis nesta categoria.')}</p></div></section>${children.length?`<div class="sc-sibling-grid">${children.map(c=>`<a class="sc-sibling-card" href="/servicos/?familia=${encodeURIComponent(fam.slug)}&categoria=${encodeURIComponent(c.slug)}"><div class="sc-sibling-copy"><strong>${esc(c.nome)}</strong><span>${categoryServices(c.id).length} serviço(s) publicado(s)</span><i class="sc-sibling-arrow">→</i></div></a>`).join('')}</div>`:''}<div class="sc-category-heading"><h2>Serviços publicados</h2><p>${rows.length?'Escolha um serviço para solicitar atendimento.':'Esta categoria está estruturada, mas ainda não possui serviços publicados.'}</p></div>${rows.length?`<section class="sc-service-results"><div class="sc-services-grid">${rows.map(p=>{const m=media.get(p.id),desc=plain(p.short_description||p.descricao||'Solicite um orçamento para este serviço.');return `<article class="sc-service-card"><div class="sc-service-media">${m?.url?`<img src="${esc(m.url)}" alt="${esc(m.alt_text||p.nome)}" loading="lazy">`:''}</div><div class="sc-service-copy"><h4>${esc(p.nome)}</h4><p>${esc(desc)}</p><a class="sc-cta" href="https://wa.me/553230253588?text=${encodeURIComponent(`Olá! Vim pelo site da Croma e gostaria de solicitar orçamento para ${p.nome}.`)}" target="_blank" rel="noopener noreferrer">Solicitar orçamento →</a></div></article>`}).join('')}</div></section>`:'<div class="sc-empty">Os serviços desta categoria serão liberados individualmente após a consolidação do catálogo.</div>'}</div>`;
}

try{
  const data=await loadPublicCatalog('servico');families=data.families;categories=data.categories;services=data.items;
  if(categorySlug)await renderCategory();else renderHome();
}catch(e){console.error('Falha ao carregar catálogo público de serviços.',e);root.innerHTML='<div class="service-catalog"><div class="sc-empty">Não foi possível carregar os serviços agora.</div></div>'}
